...
void lcd_init(void)
{
    ...
    else
    {
        ...
        /* FMC时钟来自HCLK, 假定频率最大为192Mhz（超频时，按192才能得到48M的USB时钟）
         * 寄存器清零
        ...
        LCD_FMC_BTRX |= 15 << 0;    /* 地址建立时间(ADDSET)为15个HCLK 1/192M = 5.2ns * 15 = 78ns */

        /* 因为液晶驱动IC的读数据的时候，速度不能太快,尤其是个别奇葩芯片 */
        LCD_FMC_BTRX |= 70 << 8;    /* 数据保存时间(DATAST)为70个HCLK = 5.2ns * 70 = 364ns */

        /* 写时序控制寄存器 */
        LCD_FMC_BWTRX |= 0 << 28;   /* 模式A */
        LCD_FMC_BWTRX |= 10 << 0;   /* 地址建立时间(ADDSET)为10个HCLK = 5.2ns * 10 = 52ns */

        /* 某些液晶驱动IC的写信号脉宽，最少也得50ns。 */
        LCD_FMC_BWTRX |= 10 << 8;   /* 数据保存时间(DATAST)为10个HCLK = 5.2ns * 10 = 52ns */
    ...
    if (lcddev.id == 0X7789 )
    {
        ...
        LCD_FMC_BWTRX |= 5 << 0;        /* 地址建立时间(ADDSET)为5个HCLK = 26ns */
        LCD_FMC_BWTRX |= 5 << 8;        /* 数据保存时间(DATAST)为5个HCLK = 26ns */
    }
    else if (lcddev.id == 0X5510 || lcddev.id == 0X1963)
    {
        ...
        LCD_FMC_BWTRX &= ~(0XF << 0);   /* 地址建立时间(ADDSET)清零 */
        LCD_FMC_BWTRX &= ~(0XF << 8);   /* 数据保存时间清零 */
        LCD_FMC_BWTRX |= 2 << 0;        /* 地址建立时间(ADDSET)为2个HCLK = 10.4ns */
        LCD_FMC_BWTRX |= 2 << 8;        /* 数据保存时间(DATAST)为2个HCLK = 10.4ns */
    }
    else if (lcddev.id == 0X7796 || lcddev.id == 0X9341 || lcddev.id == 0X9806 ||lcddev.id == 0X5310)
    {
        /* 重新配置写时序控制寄存器的时序 */
        LCD_FMC_BWTRX &= ~(0XF << 0);   /* 地址建立时间(ADDSET)清零 */
        LCD_FMC_BWTRX &= ~(0XF << 8);   /* 数据保存时间清零 */
        LCD_FMC_BWTRX |= 3 << 0;        /* 地址建立时间(ADDSET)为3个HCLK = 15.6ns */
        LCD_FMC_BWTRX |= 3 << 8;        /* 数据保存时间(DATAST)为3个HCLK = 15.6ns */
    }
...
void display_bitmap_on_lcd(const unsigned char bitmap_bytes[], int bitmap_width, int bitmap_height,uint32_t color,int x_offset,int y_offset) {
    bitmap_width = bitmap_width / 8;
    for (int iy = 0; iy < bitmap_height; ++iy) 
    {
        for (int ix = 0; ix < bitmap_width; ++ix) 
        {
            for (int k = 0; k < 8; ++k) 
            {
                if (bitmap_bytes[iy * bitmap_width + ix] & (0x80 >> k)) 
                {
                    int x = x_offset + ix * 8 + k;
                    int y = y_offset + iy;
...
void display_RGB565_on_lcd(const unsigned char bitmap_bytes[], int bitmap_width, int bitmap_height) {
    int x_offset = (800 - bitmap_width)/2; 
    int y_offset = (480 - bitmap_height)/2; 
    for (int iy = 0; iy < bitmap_height; ++iy) 
    {
        for (int ix = 0; ix < bitmap_width; ix++) 
        {
            int x = x_offset + ix;
            int y = y_offset + iy;
            uint32_t color = bitmap_bytes[iy * bitmap_width*2 + ix*2 + 1]*256 + bitmap_bytes[iy * bitmap_width*2 + ix*2];
            lcd_draw_point(x, y, color);
        }
    }
}
