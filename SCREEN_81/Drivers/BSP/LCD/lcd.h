...
#define LCD_CS_GPIO_CLK_ENABLE()        do{ RCC->AHB1ENR |= 1 << 3; }while(0)   /* 所在IO口时钟使能 */

#define LCD_RS_GPIO_PORT                GPIOD
#define LCD_RS_GPIO_PIN                 SYS_GPIO_PIN13
#define LCD_RS_GPIO_CLK_ENABLE()        do{ RCC->AHB1ENR |= 1 << 3; }while(0)   /* 所在IO口时钟使能 */
...
#define LCD_FMC_BCRX        FMC_Bank1->BTCR[(LCD_FMC_NEX - 1) * 2]          /* BCR寄存器,根据LCD_FMC_NEX自动计算 */
#define LCD_FMC_BTRX        FMC_Bank1->BTCR[(LCD_FMC_NEX - 1) * 2 + 1]      /* BTR寄存器,根据LCD_FMC_NEX自动计算 */
#define LCD_FMC_BWTRX       FMC_Bank1E->BWTR[(LCD_FMC_NEX - 1) * 2]         /* BWTR寄存器,根据LCD_FMC_NEX自动计算 */
