 ...
 *
 *              外部晶振为 25M的时候, 推荐值: plln = 360, pllm = 25, pllp = 2, pllq = 8.
 *              得到:Fvco = 25 * (336 / 25) = 360Mhz
 *                   Fsys = pll_p_ck = 360 / 2 = 180Mhz
 *                   Fq   = pll_q_ck = 360 / 8 = 45Mhz(使用USB时，需设置plln=384，即可得到48Mhz频率, 此时主频为192M)
 *
 *              F429默认需要配置的频率如下:
...
uint8_t sys_clock_set(uint32_t plln, uint32_t pllm, uint32_t pllp, uint32_t pllq)
{
    ...
    else
    {
        ...
        retry = 0;
        while (swsval != 2)                     /* 等待成功将系统时钟源切换为pll_p_ck */
        {
 ...
 * @param       pllp: PLL1的p分频系数(PLL之后的分频), 取值范围:2,4,6,8.(仅限这4个值!).
